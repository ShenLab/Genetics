...
my $file_map="$path/Resources/mappability/hg19_152bp_mappability.bed.gz";
my $fmcap="$path/PSAP/Data/exon_transcript.MCAP.gene.mappability.bed.gz";
my $fmeta="$path/PSAP/Data/exon_transcript.dbnsfp30a.gene.bed.gz";
my $feigen="$path/Resources/Eigen/xioniti01.u.hpc.mssm.edu/v1.1/Eigen_hg19_coding_04092016_sim.tab.gz";
my $fcnn="/home/local/users/jw/resources/Annotations/deepmiss.sort.hg19.txt.gz";
...
#	exit;	
		my %gene_rate=();
		my @p_cnn_nms=();
		for(my $v=0.05;$v<0.95;$v=$v+0.05){
		  push(@p_cnn_nms,$v);
		}
		push(@p_cnn_nms,0.56);
		print join("\t",@p_cnn_nms)."\n";
		open OUT, ">$fout";
	...
	sub load_dmis {
		...
		while(my $line=<INM>){
			...
			if($sets[10] eq "D" && $sets[23]>=15 && $sets[28] eq "T" ){
			   $pdmis{$key}=1;
			}
			if($sets[8] eq "D" && $sets[23]>=15 && $sets[28] eq "T" ){
			   $phdmis{$key}=1;
			}
			if($sets[10] eq "D" ){
			   $pp2hvar{$key}=1;
		...
		while(my $line=<INE>){
			my @sets=split(/\s+/,$line);
			if($sets[3] eq ""){print $line."\n"};
			$eigen_phred{join(":",@sets[(1,2,3)])}=$sets[4];  ## >10 >15
		    $eigen_pc_phred{join(":",@sets[(1,2,3)])}=$sets[6]; ## >10
		...
		return (\%mcap,\%meta,\%lr,\%pdmis,\%phdmis,\%pcadd,\%pp2hvar,\%fathmm,\%eigen_phred,\%eigen_pc_phred,\%cnn_map);
	}
	...
	sub callinfo {
		...
		my($addr_mcap,$addr_meta,$addr_lr,$addr_pdmis,$addr_phdmis,$addr_cadd,$addr_pp2hvar,
...
#	print $key."\t".$value."\n";
	...
	print "Input perl Mutation_rate_20180814.pl transcript_file outname filter_with_mappability\n";
